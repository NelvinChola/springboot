          ...
          .then(data => {
              ...
              .then(issues => {
                ...
                var name= equip=== undefined?"unknwown":equip.name;
                  rows += `<tr ><td> `+ this.id + `</td><td>` + name+  `</td><td>` + this.description + `</td><td>` + this.location + `</td><td>` + this.status+ `<p style="font-size:16px;padding:5px;display:inline-block; text-decoration:underline;color:green;" id=${this.id} onClick="logStuff()" data-bs-toggle="modal" data-bs-target="#issueModal">Edit</p>`  + `</td><td>` + this.date + `</td></tr>`;
